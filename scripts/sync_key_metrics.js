...
const validateMetric = (metricId, metric) => {
  ...
  if (metricId === "server_cpu") {
    ...
    assert(/不是纯 CPU 收入指数/.test(metric.sourceNote), issues, "server_cpu sourceNote must explicitly state proxy limitation");
  }
...
const renderAudit = ({ source, metrics }, validation, sourceChecks) => {
  const sourceCheckRows = sourceChecks.length === 0
    ...
    : sourceChecks.map((item) => `- ${item.ok ? "通过" : "警告"} · ${item.label} · ${item.status} · ${item.url}${item.error ? ` · ${item.error}` : ""}`).join("\n");
  ...
  const metricRows = Object.entries(metrics).map(([metricId, metric]) => {
    ...
    return [
      `## ${metricId}`,
      `- 状态: ${metric.status}`,
      `- 数据日期: ${metric.updatedAt}`,
      `- 更新频率: ${metric.cadence}`,
      `- 来源数量: ${metric.sources.length}`,
      `- 校验结果: ${issues.length === 0 ? "通过" : `警告 (${issues.length})`}`,
      issues.map((issue) => `  - ${issue}`).join("\n"),
      `- 口径限制: ${metric.sourceNote}`,
    ].filter(Boolean).join("\n");
  }).join("\n\n");
